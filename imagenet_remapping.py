...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # model = BaseMobileNetV2(n_class=args.n_classes, width_mult=1.5)
    model = ImageNetModel(
    ...
    if args.remapping:
        writer_dir = '{}/runs/{}/{}-{}-{}_remapping'.format(args.output, args.lr, args.arch, args.n_classes, args.ratio)
    else:
    ...
    if args.remapping:
        ...
        load_path='exp/checkpoint/0.2/mobilenet-100-0.2_model_best_train.pth.tar', 
    ...
    if args.resume:
        filename = 'exp/checkpoint/0.02/mobilenet15-100-0.4-remapping_final_train.pth.tar'
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if not args.multiprocessing_distributed or (args.multiprocessing_distributed
            ...
            save_path = '{}/checkpoint/{}'.format(args.output, args.lr)
            if not os.path.exists(save_path):
                os.makedirs(save_path, exist_ok=True)
            if args.remapping:
                filename = '{}/{}-{}-{}-remapping_final_train.pth.tar'.format(save_path, args.arch, args.n_classes, args.ratio)
            else:
                filename = '{}/{}-{}-{}_final_train.pth.tar'.format(save_path, args.arch, args.n_classes, args.ratio)
            save_checkpoint({
                'epoch': epoch + 1,
                'arch': args.arch,
